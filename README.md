...
>
> **Note on rule numbering:** rule ID `184666` above refers to this project's own rewritten rule in the process-anomaly family. The Wazuh server's default ruleset also ships a rule numbered `184666` with different logic (the stock `svchost.exe` / `sysmon.image` check described above) - this is a pre-existing ID collision between the two rulesets, not a duplicate or conflicting definition of the same rule. It also explains why this project's own `0330` -> `0850` rewrite could independently reference `if_sid>92000` and get silently discarded during early development (see "Failure 1" above), while the stock `184666` - untouched, unrelated code - never had that problem: they are two different rules that happen to share a number. Same category of collision as rule `92153` (EID 7) further below.
>
